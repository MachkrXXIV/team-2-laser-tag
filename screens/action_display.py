...
class ActionDisplay(ttk.Frame):
    ...
    def display_scoreboard(self):
        ...
        red_team_score = sum(player.points for player in game_manager.red_team.players.values())
        green_team_score = sum(player.points for player in game_manager.green_team.players.values())
        higher_score_team = "Red" if red_team_score > green_team_score else "Green"
        higher_score_team_label = self.red_column if higher_score_team == "Red" else self.green_column
        ...
        for team, entries in self.player_entries.items():
            ...
            for player_id, (user_id_entry, player_name_entry) in entries.items():
                ...
                score_label = ttk.Label(
                    player_label_frame, text=f"Score: {player_score}", style="Scoreboard.TLabel"
                )
