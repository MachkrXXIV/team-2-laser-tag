...
class PlayerEntry(ttk.Frame):
    def __init__(self, parent: ttk.Frame, controller: "App", database: Database):
        ...
        self.style.configure(
            "Red.TLabelframe", background="#800000", bordercolor=" "
        )  # Darker red
    ...
    def create_player_entries(self, parent_frame: tk.LabelFrame, team: str):
        ttk.Label(parent_frame, text="User ID".format(team)).grid(
            row=1, column=3, padx=5, pady=(5, 0), sticky="w", columnspan=2
        )
        ttk.Label(parent_frame, text="Player Names").grid(
            row=1, column=5, padx=5, pady=(5, 0), sticky="w", columnspan=2
        )
        ...
        def add_player(team):
            def inner_add_player():
                ...
                if player_id is not None and player_id >= 0:
                    ...
                    equipment_id = simpledialog.askinteger(
                        "Equipment ID", "Enter Equipment ID"
                    )

                    udp.broadcast_code(equipment_id)
                    player.equipment_id = equipment_id

                    user_id_entry = ttk.Label(
                        parent_frame, text=player.id, state="readonly"
                    )
                    user_id_entry.grid(
                        row=self.row_counters[team],
                        column=3,
                        sticky="w",
                        padx=5,
                        pady=5,
                    ...
                    player_name_entry = ttk.Label(
                        parent_frame, text=player.name, state="readonly"
                    )
                    player_name_entry.grid(
                        row=self.row_counters[team],
                        column=5,
                        padx=5,
                        pady=5,
                    ...
                    self.player_entries[team][player_id] = (
                        user_id_entry,
                        player_name_entry,
                    )
        ...
        ttk.Button(parent_frame, text="Add Player", command=add_player(team)).grid(
            row=17, column=2, columnspan=5, pady=5
        )
